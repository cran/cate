...
samples were taken from different regions of the brain of each individual, and one aliquot of each sample was sent to each of 3 laboratories for analysis. There were two different microarray platforms used by these labs.
...
@
Under the null hypothesis that the gene is unrelated to the gender, the corresponding t-statistic is expected to follow a t-distribution with $n-1$ degrees of freedom. Since $n=84$ is fairly large in this study, this t-distribution is very close to the standard normal distribution $\mathrm{N}(0,1)$. However, the empirical distribution of \texttt{t.stats} is shown in the next histogram (code not echoed):
...
@
The \texttt{"bcv"} method tends to estimate more weak factors and takes longer time than \texttt{"ed"}. We recommend to use \texttt{"bcv"} method for most datasets; see \citet{wang2015bcv} for more detail.
...
\begin{description}
\item[\texttt{X}:] primary treatment variable(s) whose effects are of interest.
\item[\texttt{Y}:] response matrix (e.g.\ gene expression).
\item[\texttt{X.nuis}:] nuisance covariate(s) to include in the regression whose effects are not of interest. Generally \texttt{X.nuis} includes a column of intercept, and can further include the known batch variables (e.g. the matrix \texttt{Z} in the \texttt{gender.sm} dataset) and other demographics variables such as age or gender.
