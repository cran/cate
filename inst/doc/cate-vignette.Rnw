...
samples were taken from different regions of the brain of each individual, and one aliquot of each sample was sent to each of 3 laboratories for analysis. There were two different microarray platforms used by these labs. The dataset was downloaded from \href{http://www.stat.berkeley.edu/~johann/ruv/}.
...
\begin{description}
\item[\texttt{formula}:] R formula indicating primary and nuisance predictors, which are separated by "|". Example: \texttt{formula = ~ treatment | batch.label}.
